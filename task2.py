...
def get_network_components(old_components, edge_to_break):
    '''
    :param old_network_components: calculate network components and send - dict of adjlist
    :param edge_to_break: edge to break and then calc network components
    :return: new_net_comps - list of RDDs
    '''
    # print("(get_network_components) Old components [find new comps with these]", old_components)

    # print("(get_network_components) Edge to break:", edge_to_break)
    old_network_components = []
    for compnt in old_components:
        old_network_components.append(compnt.collectAsMap())
    ...
    if edge_to_break is None:
        for component in old_network_components:
            ...
            while remaining_vertices:
                ...
                new_components.append(sc.parallelize(new_component))
        ...
        for component in old_network_components:
            if edge_to_break[0] in component.keys():
                ...
                if len(component[edge_to_break[0]]) == 0:
                    ...
                    new_components.append(sc.parallelize([(edge_to_break[0], [])]))
            if edge_to_break[1] in component.keys():
                ...
                if len(component[edge_to_break[1]]) == 0:
                    ...
                    new_components.append(sc.parallelize([(edge_to_break[1], [])]))

        # print("(get_network_components) num of Components after removing edge: ", len(old_network_components))

        # performing bfs and getting all components.
        # new_components = []
        for component in old_network_components:
            # perform bfs and keep adding RDDs of new components to new_components
            vertices = set(component.keys())
            ...
            while remaining_vertices:
                ...
                new_components.append(sc.parallelize(new_component))
                remaining_vertices = vertices.difference(visited)
    # print_list = [x.collect() for x in new_components]
    # print("(get_network_components) New components found from old comps: ", new_components)
...
with open(str("task2a_xia.txt"), "w") as file:
...
while True:
    ...
    for component in network_components:
        ...
        if btw_cent:
            ...
            edge_break_candidates.append(btw_cent[0])
    # print("-------------------------")
    print("Sigma_nodes(components): ", num_nodes)
    edge_to_break = tuple()
    if edge_break_candidates:
    ...
    if new_modularity > max_modularity:
        ...
        # final_communities = network_components[:]
        # break
    network_components = new_network_components[:]
